...
import itertools
# vendored: optimized_attention shim
def optimized_attention(q, k, v, heads, mask=None, skip_reshape=False, **kwargs):
    ...
    if mask is not None:
        ...
        if attn_mask is not None:
            ...
            scores = scores + attn_mask.to(q.dtype)
...
class AceStepAttention(nn.Module):
    def __init__(
        ...
        self.q_norm = nn.RMSNorm(head_dim, eps=rms_norm_eps, dtype=dtype, device=device)
        self.k_norm = nn.RMSNorm(head_dim, eps=rms_norm_eps, dtype=dtype, device=device)
...
class AceStepDiTLayer(nn.Module):
    def __init__(
        ...
        self.self_attn_norm = nn.RMSNorm(hidden_size, eps=rms_norm_eps, dtype=dtype, device=device)
        ...
        self.cross_attn_norm = nn.RMSNorm(hidden_size, eps=rms_norm_eps, dtype=dtype, device=device)
        self.cross_attn = AceStepAttention(
            hidden_size, num_heads, num_kv_heads, head_dim, rms_norm_eps,
            is_cross_attention=True, dtype=dtype, device=device, operations=operations
        )

        self.mlp_norm = nn.RMSNorm(hidden_size, eps=rms_norm_eps, dtype=dtype, device=device)
    ...
    def forward(
        ...
        modulation = self.scale_shift_table.to(dtype=temb.dtype, device=temb.device) + temb
...
class AceStepEncoderLayer(nn.Module):
    def __init__(
        ...
        self.input_layernorm = nn.RMSNorm(hidden_size, eps=rms_norm_eps, dtype=dtype, device=device)
        self.post_attention_layernorm = nn.RMSNorm(hidden_size, eps=rms_norm_eps, dtype=dtype, device=device)
        self.mlp = MLP(hidden_size, intermediate_size, dtype=dtype, device=device, operations=operations)
...
class AceStepLyricEncoder(nn.Module):
    def __init__(
        ...
        self.norm = nn.RMSNorm(hidden_size, eps=rms_norm_eps, dtype=dtype, device=device)
...
class AceStepDiTModel(nn.Module):
    def __init__(
        ...
        self.norm_out = nn.RMSNorm(hidden_size, eps=rms_norm_eps, dtype=dtype, device=device)
    ...
    def forward(
        ...
        shift, scale = (self.scale_shift_table.to(dtype=temb.dtype, device=temb.device) + temb.unsqueeze(1)).chunk(2, dim=1)
...
class AttentionPooler(nn.Module):
    ...
    def forward(self, x):
        ...
        special = self.special_token.to(device=x.device, dtype=x.dtype).expand(B, T, 1, -1)
...
class FSQ(nn.Module):
    def __init__(
        ...
        if requires_projection:
            self.project_in = nn.Linear(self.dim, self.codebook_dim, device=device, dtype=dtype)
            self.project_out = nn.Linear(self.codebook_dim, self.dim, device=device, dtype=dtype)
        else:
    ...
    def bound(self, z):
        levels_minus_1 = (self._levels.to(device=z.device, dtype=z.dtype) - 1)
    ...
    def codes_to_indices(self, zhat):
        zhat_normalized = (zhat + 1.) / (2. / (self._levels.to(device=zhat.device, dtype=zhat.dtype) - 1))
        return (zhat_normalized * self._basis.to(device=zhat.device, dtype=zhat.dtype)).sum(dim=-1).round().to(torch.int32)
...
class ResidualFSQ(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        if requires_projection:
            self.project_in = nn.Linear(dim, codebook_dim, device=device, dtype=dtype)
            self.project_out = nn.Linear(codebook_dim, dim, device=device, dtype=dtype)
        else:
            self.project_in = nn.Identity()
            self.project_out = nn.Identity()
    ...
    def get_output_from_indices(self, indices, dtype=torch.float32):
        ...
        for i, layer in enumerate(self.layers):
            idx = indices[..., i].long()
            codes = F.embedding(idx, layer.implicit_codebook.to(device=idx.device, dtype=dtype))
    ...
    def forward(self, x):
        ...
        if hasattr(self, 'soft_clamp_input_value'):
            sc_val = self.soft_clamp_input_value.to(device=x.device, dtype=x.dtype)
...
class AudioTokenDetokenizer(nn.Module):
    def __init__(
        ...
        self.norm = nn.RMSNorm(hidden_size, dtype=dtype, device=device)
    ...
    def forward(self, x):
        ...
        x = x + self.special_tokens.expand(B, T, -1, -1).to(device=x.device, dtype=x.dtype)
